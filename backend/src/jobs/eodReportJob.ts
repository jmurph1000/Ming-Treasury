...
/**
 * Compute the 6:00 PM ET timestamp for a given date string (YYYY-MM-DD).
 * Returns an ISO string representing 18:00 America/New_York on that date.
 */
function get6pmEtIso(dateStr: string): string {
  // Build a Date for 18:00 ET on the given date.
  // We set TZ=America/New_York at process start, so local time IS ET.
...
/**
 * Format an ISO timestamp as a human-readable ET string for report footers.
 */
...
export async function runEodReportJob(manualUserId?: string, reportDate?: string, isScheduled: boolean = false): Promise<void> {
  try {
    ...
    // Determine the correct generated_at timestamp:
    // - Scheduled or catch-up-as-scheduled: use 6 PM ET on the target date
    // - Retroactive catch-up (startup): use 6 PM ET on the target date
    // - Manual/startup for today: use actual current time
    let generatedAt: string;
    if (isScheduled || isRetroactive) {
      generatedAt = get6pmEtIso(targetDate);
    } else {
    ...
    }, { isRetroactive: !!isRetroactive, isScheduled, generatedByName, generatedAt });
...
function buildEodHtml(
  ...
  return `
    <div style="font-family:Arial,sans-serif;max-width:1000px;margin:0 auto;">
      ...
      ${ctx.isRetroactive && !ctx.isScheduled ? `<div style="background:#fef3c7;border:1px solid #f59e0b;border-radius:6px;padding:10px 14px;margin-bottom:16px;font-size:13px;color:#92400e;">
        ...
        ${ctx.isScheduled
          ...
          : ctx.isRetroactive
            ...
            : ctx.generatedByName
              ...
              : `Generated by the Gusto Treasury Portal at ${formatEtTimestamp(ctx.generatedAt)}.`
        }
      </p>
    </div>
  `;
